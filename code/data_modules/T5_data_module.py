...
class DataModuleForT5(LightningDataModule):
    ...
    def setup(self, stage: str):
        print("⚡", "using setup", "⚡")
        self.train_dataset = DatasetForT5(self.__train_dataset, self.tokenizer,
                                          input_length=self.max_input_length,
                                          output_length=self.max_output_length,
                                          source_prefix=self.prefix)
        self.val_dataset = DatasetForT5(self.__eval_dataset, self.tokenizer,
                                        input_length=self.max_input_length,
                                        output_length=self.max_output_length,
                                        source_prefix=self.prefix)
        self.test_dataset = DatasetForT5(self.__test_dataset, self.tokenizer,
                                         input_length=self.max_input_length,
                                         output_length=self.max_output_length,
                                         source_prefix=self.prefix)
